...
static void usage(const char* prog, std::ostream& out) {
    out << "Usage:\n"
        ...
        << "  " << prog << " <graph_file> sssp1d <source> [--delta D] [--lcc-info <file>] [--output <path>] [--no-output]\n"
        << "  " << prog << " <graph_file> sssp2d <source> [--delta D] [--lcc-info <file>] [--output <path>] [--no-output]\n";
}
...
// SSSP flags: BFS flags + --delta D.
static bool parse_sssp_flags(int argc, char** argv, int start, BFSOpts& opts,
                             double& delta, int rank) {
    for (int i = start; i < argc; i++) {
        if (!std::strcmp(argv[i], "--delta") && i + 1 < argc) {
            delta = std::stod(argv[++i]);
        } else if (!std::strcmp(argv[i], "--lcc-info") && i + 1 < argc) {
            opts.lcc_info = argv[++i];
        } else if (!std::strcmp(argv[i], "--output") && i + 1 < argc) {
            opts.output_file = argv[++i];
            opts.output_set  = true;
...
// Writes one "<v> <dist|INF>" line per global vertex to `out`. Format matches
// reference/dijkstra.cpp so the two outputs can be diffed directly.
static void write_sssp_distances(std::ostream& out, const std::vector<double>& all_d,
                                 int64_t n_global) {
    const double INF = std::numeric_limits<double>::infinity();
    for (int64_t v = 0; v < n_global; v++) {
        if (all_d[v] == INF) out << v << " INF\n";
        else                 out << v << " " << all_d[v] << "\n";
    }
}

static int run_sssp1d(const std::string& filename, int rank, int p, int argc, char** argv) {
    if (argc < 4) {
        if (rank == 0) std::cerr << "Error: sssp1d requires <source>\n";
        return 1;
    }
    int64_t source = std::stoll(argv[3]);

    BFSOpts opts;
    double  delta = 10.0;  // reasonable for the [1,100] weights generated by graph_utils
    if (!parse_sssp_flags(argc, argv, 4, opts, delta, rank)) return 1;

    CSRGraph full;
    if (rank == 0) full = load_snap_graph_serial(filename, /*assign_weights=*/true);
    CSRGraph g = distribute_graph_1d(full, MPI_COMM_WORLD);
    ...
    // Time externally: DeltaStepping doesn't expose a timing struct of its own.
    MPI_Barrier(MPI_COMM_WORLD);
    double t0 = MPI_Wtime();
    DeltaStepping ds(g, source, delta, MPI_COMM_WORLD);
    ds.run();
    ...
    if (rank == 0) {
        double teps = (t_total > 0.0) ? (double)m_lcc / t_total : 0.0;
        std::cout << "[sssp1d] source=" << source << " ranks=" << p
                  << " delta=" << delta
    ...
    // Gather local distance slices on rank 0. 1D block layout has vertex_start
    // monotonically increasing with rank, so rank-order concatenation is
    // global-vertex-ordered.
    int local_count = (int)g.n_local;
    std::vector<int> counts, displs;
    if (rank == 0) { counts.resize(p); displs.resize(p); }
    MPI_Gather(&local_count, 1, MPI_INT, counts.data(), 1, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    MPI_Gatherv(ds.distances().data(), local_count, MPI_DOUBLE,
                all_d.data(), counts.data(), displs.data(), MPI_DOUBLE,
                0, MPI_COMM_WORLD);

    if (rank == 0) {
        std::string output = opts.output_set ? opts.output_file
                                             : default_output_path("sssp1d", filename, source);
        std::ofstream fout(output);
        if (!fout.is_open()) {
            std::cerr << "Error: cannot open output file: " << output << "\n";
            return 1;
        }
        write_sssp_distances(fout, all_d, g.n_global);
        std::cerr << "Wrote SSSP distances to " << output << "\n";
    }
    return 0;
}

static int run_sssp2d(const std::string& filename, int rank, int p, int argc, char** argv) {
    if (argc < 4) {
        if (rank == 0) std::cerr << "Error: sssp2d requires <source>\n";
        return 1;
    }
    int64_t source = std::stoll(argv[3]);
    ...
    if (R * R != p) {
        if (rank == 0)
            std::cerr << "Error: sssp2d requires a square number of ranks (got " << p << ")\n";
        return 1;
    }
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    double t0 = MPI_Wtime();
    DeltaStepping2D ds(g, source, delta, MPI_COMM_WORLD);
    ds.run();
    double t_local = MPI_Wtime() - t0;
    double t_total = 0.0;
    MPI_Reduce(&t_local, &t_total, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        double teps = (t_total > 0.0) ? (double)m_lcc / t_total : 0.0;
        std::cout << "[sssp2d] source=" << source << " ranks=" << p
                  << " delta=" << delta
                  << " n_lcc=" << n_lcc << " m_lcc=" << m_lcc
                  << " total_time=" << t_total
                  << " teps=" << teps << "\n";
    ...
    // 2D vector partition: vec_start is monotonic with rank (rows-major over
    // grid, with each row split among proc-cols), so rank-order concatenation
    // is global-vertex-ordered. Same property bfs2d relies on.
    int local_count = (int)(ds.vec_end() - ds.vec_start());
    std::vector<int> counts, displs;
    if (rank == 0) { counts.resize(p); displs.resize(p); }
    MPI_Gather(&local_count, 1, MPI_INT, counts.data(), 1, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        std::string output = opts.output_set ? opts.output_file
                                             : default_output_path("sssp2d", filename, source);
        std::ofstream fout(output);
        if (!fout.is_open()) {
            std::cerr << "Error: cannot open output file: " << output << "\n";
            return 1;
...
int main(int argc, char** argv) {
    ...
    try {
        if (mode == "stats1d")
            ...
            ret = run_sssp1d(filename, rank, p, argc, argv);
        else if (mode == "sssp2d")
            ret = run_sssp2d(filename, rank, p, argc, argv);
        else {
            if (rank == 0) {
